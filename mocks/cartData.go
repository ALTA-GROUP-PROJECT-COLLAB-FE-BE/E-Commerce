...
// CartData is an autogenerated mock type for the Data type
...
func (_m *CartData) SelectData(limit int, offset int, idFromToken int) ([]cart.Core, error) {
	ret := _m.Called(limit, offset, idFromToken)

	var r0 []cart.Core
	if rf, ok := ret.Get(0).(func(int, int, int) []cart.Core); ok {
...
// UpdateDataDB provides a mock function with given fields: qty, idCart, idFromToken
func (_m *CartData) UpdateDataDB(qty int, idCart int, idFromToken int) (int, error) {
	ret := _m.Called(qty, idCart, idFromToken)

	var r0 int
	if rf, ok := ret.Get(0).(func(int, int, int) int); ok {
		r0 = rf(qty, idCart, idFromToken)
	} else {
		r0 = ret.Get(0).(int)
	}
